        ...
        this.columns = [
            {
                ...
                width: 180,
                render: (text, record) => {
                    return <Tooltip placement="topLeft" title={text}><a onClick={()=>this.props.history.push(`${record.status == 2 ? "/policyText" : "/policyPreview"}/${record.id}`)} target="_blank">{text.length < 8 ? text : text.substr(0,8)+"..."}</a></Tooltip>
                }
            },
            {
                title: '所属层级',
